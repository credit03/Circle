...
import com.guoyi.circle.utils.DensityUtil;
...
public class MultiImageView extends LinearLayout {
     ...
     * 单张图片时，是否显示小图方式
     */
    private boolean showThumbnailsMode = false;

    public void setShowThumbnailsMode(boolean showThumbnailsMode) {
        this.showThumbnailsMode = showThumbnailsMode;
    }
     ...
     */
    public void setStringList(List<String> lists) throws IllegalArgumentException {
        if (lists == null) {
        ...
        for (String s : lists) {
            ...
            bean.setUrl(s);
            bean.setLocalUrl(true);
            beanList.add(bean);
        }
        setList(beanList);
    }

    public void setList(List<ImageBean> lists) throws IllegalArgumentException {
        if (lists == null) {
            throw new IllegalArgumentException("imageList is null...");
        }
        imagesList = lists;
    ...
    private void initImageLayoutParams() {
        ...
        onePicPara = new LayoutParams(320, 320);
    ...
    private ImageView createImageView(int position, final boolean isMultiImage) {
        ImageBean photoInfo = imagesList.get(position);
